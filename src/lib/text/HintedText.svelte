<script lang="ts">
    ...
    function getType(char: string) {
        ...
        else if (hintSettings.punctuation.includes(char)) return 1;
        return 2;
    }
    ...
    function recalculatePopup() {
        ...
        if (writing.horizontal) {
            x += nth.offsetWidth / 2;
            y += nth.offsetHeight;
        } else {
            y += nth.offsetHeight / 2;
            if (writing.ltr) x += nth.offsetWidth;
        }
        ...
        popup = addPopup({
            settings: {
                px: x,
                py: y,
                rotation: writing.horizontal ? 0 : writing.ltr ? 3 : 1,
                reverseSnap: writing.horizontal && !writing.ltr,
                onescape: async () => {
                    if (popup === -1) return;
...
<OrientatedText {writing}>
    <span
        class="hinted"
        class:edit
        class:showHints
        class:vert={!writing.horizontal}
        class:pad={!writing.upright || writing.horizontal}
